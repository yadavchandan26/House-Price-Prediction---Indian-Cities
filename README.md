...
- One-hot encode `Location` instead of manually ranking it
- Scale numeric features (`StandardScaler`)
- Evaluate on the test set with RMSE/MAE in addition to R²
- Compare against Ridge/Lasso or a tree-based model
- Add cross-validation instead of a single fixed split
- Save the trained model with `joblib` and add a separate inference script
